...
- `npm run nursys:ping` — smoke-test the live NCSBN e-Notify credentials. `npm run nursys:rotate` rotates the Nursys password via `/api/nursys/change-password`. All `tsx` scripts in `scripts/` load `.env.local` via `--env-file`.
- Path alias: `@/*` → `./src/*`.
...
RLS is enabled on every table; `supabase/schema.sql` is the source of truth for both the schema and the policies. For a brand-new project, run `schema.sql` once then apply `supabase/migrations/*.sql` in numbered order; every schema change must also land as a new numbered migration so prod can be upgraded after testing on the dev project (the two-Supabase-project workflow is in `DEPLOY.md`). Two triggers do non-obvious work that downstream code relies on:
...
**Notification sweep cron.** `/api/nursys/cron/check-notifications` runs daily at 06:00 UTC. It calls `submitNotificationLookup` for a 1-day window, polls within the function deadline, and clears `license_verified` (forcing re-verification) on any nurse whose change matches `STATUS_CHANGE_TRIGGERS_RECHECK` in `lib/nursys.ts`. The route requires `Authorization: Bearer ${CRON_SECRET}`. **Cron scheduling has two homes** — `vercel.json` (legacy) and `.github/workflows/cron-*.yml` (current): production runs on DigitalOcean App Platform, which has no built-in scheduler, so the GitHub Actions workflows `curl` the endpoints on schedule. Edit the GitHub workflow, not `vercel.json`, when changing a cron.
...
- **Contract Check (single-quote assessment).** `/nurse/contract-check` + `/api/ledger/contract-check` turn one recruiter quote into an instant assessment with zero comparison data: `contract-check.ts` computes the blended hourly rate, the bill-rate capture (`blended ÷ bill_rate_cents`, persisted to `ledger_contracts.bill_rate_capture`), and four deterministic red flags (low base + inflated stipends, housing/meals over the GSA ceiling, blended figure with no breakdown, undisclosed bill rate) plus the tax-home day-count surface. GSA ceilings come from the seeded `gsa_per_diem_rates` table via `gsa.ts` (city → state-default → CONUS fallback). Pure and unit-tested — `tests/ledger/contract-check.test.ts`, `tests/ledger/gsa.test.ts`.
...
`NEXT_PUBLIC_*` vars are inlined into the client bundle at **build** time, so they must be set as build-time env in the deploy target — not just runtime.

## Deployment

Production and staging run on **DigitalOcean App Platform** (config-as-code in `.do/app.yaml` for prod / `.do/app.staging.yaml` for staging; `deploy_on_push: true` on `main` / `staging` branches). `.node-version` pins the buildpack's Node version. `DEPLOY.md` is the full runbook and the authority on the environment strategy: two Supabase projects (dev + prod), Stripe test-vs-live keys on one account, and **Nursys left blank outside production** because enrollment is billed per-nurse (blank creds trigger the dev fallback). The `vercel.json` crons are inert on DigitalOcean — see the cron note above.
